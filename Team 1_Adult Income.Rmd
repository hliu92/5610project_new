...
```{r}
fit.int1 <- update(fit.full,.~.+marital_new:capital_loss_cat)
lrtest(fit.int1,fit.full)
## auc 91.15%   AIC 15483

fit.int2 <- update(fit.int1,.~.+sex:marital_new)
lrtest(fit.int2,fit.int1)
## auc 91.3%   AIC 15361

fit.int3 <- update(fit.int2,.~.+capital_gain_cat:marital_new)
lrtest(fit.int3,fit.int2)
## auc 91.43%   AIC 15311

fit.int4 <- update(fit.int3,.~.+age:marital_new )
lrtest(fit.int4,fit.int3) 
## auc 91.42%   AIC 15265

fit.int5 <- update(fit.int4,.~.+hr_per_week:marital_new)
lrtest(fit.int5,fit.int4) 
## auc 91.36%   AIC 15229

fit.int6 <- update(fit.int5,.~.+occupation:marital_new  )
lrtest(fit.int6,fit.int5) 
## auc 91.36%   AIC 15204

fit.int7 <- update(fit.int6,.~.+age:type_employer   )
lrtest(fit.int7,fit.int6) 
## auc 91.41%   AIC 15188
summary(fit.int7)
...
**We choose fit.int2 as our final model after adding interaction items.**

# Deviance Goodness of Fit Test

```{r}
1-pchisq(fit.int4$deviance, fit.int4$df.residual)
...
hl700 <- hoslem.test(as.numeric(training_set$income==">50K"), fitted(fit.int4), g=700)
hl700
head(cbind(hl700$expected, hl700$observed),20)
#hl10 <- hoslem.test(as.numeric(training_set$income==">50K"), fitted(fit.int2), g=10)
...
# Deviance Residual
```{r}
plot(residuals.glm(fit_step_int,type="deviance"))
plot(residuals.glm(fit.int4,type="deviance")[1:5000])
```

# Pearson Residual
```{r}
plot(residuals.glm(fit.int4,type="pearson"))
plot(residuals.glm(fit.int4,type="pearson")[1:5000])
```
...
```{r}
plot(rstudent(fit.int4))
plot(rstudent(fit.int4)[1:5000])
...
```{r}
plot(income_trial$ID[income_rec$age >25 ],
     residuals.glm(fit.int2,type="pearson")[income_rec$age >25],xlim=c(1,30000),ylim=c(-10,10),col=2)
par(new=TRUE)
plot(income_trial$ID[income_rec$age <26],
     residuals.glm(fit.int2,type="pearson")[income_rec$age <26],xlim=c(1,30000), ylim=c(-10,10),col=4)
par(new=FALSE)

plot(income_trial$ID[income_rec$marital_new=="Married"],
     residuals.glm(fit.int2,type="pearson")[income_rec$marital_new=="Married"],xlim=c(1,30000),ylim=c(-10,10),col=2)
par(new=TRUE)
plot(income_trial$ID[income_rec$marital_new=="Alone"],
     residuals.glm(fit.int2,type="pearson")[income_rec$marital_new=="Alone"],xlim=c(1,30000), ylim=c(-10,10),col=4)
```
