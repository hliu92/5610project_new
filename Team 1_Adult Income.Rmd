...
```{r}
1-pchisq(fit.int2$deviance, fit.int2$df.residual)
...
hl700 <- hoslem.test(as.numeric(training_set$income==">50K"), fitted(fit.int2), g=700)
hl700
...
# Deviance Residual
```{r}
plot(residuals.glm(fit.int2,type="deviance"))
plot(residuals.glm(fit.int2,type="deviance")[1:5000])
```

# Pearson Residual
```{r}
plot(residuals.glm(fit.int2,type="pearson"))
plot(residuals.glm(fit.int2,type="pearson")[1:5000])
```
...
```{r}
plot(rstudent(fit.int2))
plot(rstudent(fit.int2)[1:5000])
