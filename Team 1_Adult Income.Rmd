...
unique(income[,c(3,4)]) # check relationship between 'education' and 'education_num'
...
income$capital_abseff <- income$capital_gain + income$capital_loss
ds4.2 <- ggplot(income, aes(x=capital_abseff)) + 
    geom_density(aes(fill=factor(income)),alpha=0.5) +
    labs(title="Density of Income by Absolute Capital Gain (Loss) Effect", 
       y="Density of Income", 
       x="Absolute Capital Gain (Loss) Effect") + 
    scale_fill_manual(name = "income",
                      values = c(brewer.pal(7, "Reds")[5], brewer.pal(7, "Blues")[5]),
                      labels = c("Income <= 50K", "Income > 50K"))
plot(ds4.2)
